...
def test_ledger_process_worker_captures_selection_in_the_same_outer_pass(
    tmp_path: Path,
    monkeypatch,
) -> None:
    timestamps = tuple(START + timedelta(days=index) for index in range(1_386))
    ...
    fold = WalkForwardFold(
        ...
        fold_id="fold_001",
        train_start=timestamps[0],
        train_end=timestamps[1_259],
        test_start=timestamps[1_260],
        test_end=timestamps[1_322],
        train_source_observations=1_260,
        test_source_observations=63,
    )
    profile = load_profile("configs/profiles/xetra_v4.yaml")
    plan = SimpleNamespace(plan_hash="b" * 64, evaluation_cutoff=fold.test_end)
    dataset_key = DatasetSnapshotIdentity.from_catalog(catalog)
    ...
    def fake_outer_fold(*args, **kwargs):
        ...
        del args
        calls += 1
        kwargs["selection_sink"](1, selection)
        kwargs["prefix_evaluation_sink"](2, "candidate", prefix_evaluation)
        return SimpleNamespace(valid=True, fold_index=1)

    context = global_v4._OuterProcessContext(
        source_rows=source_rows,
        catalog=catalog,
        profile=profile,
        build_id="build-1",
        outer_runner=lambda: None,
        teacher_refitter=lambda: None,
    ...
    result, captured, prefixes = global_v4._evaluate_outer_fold_process_with_selection(fold)

    assert calls == 1
    assert result.valid is True
    assert captured is selection
    assert prefixes == {(2, "candidate"): prefix_evaluation}
    unit = WorkUnitIdentity(
        ...
        unit_parameters=(
            ("test_source_observations", "63"),
            ("train_source_observations", "1260"),
        ),
    )
    assert store.work_unit_state(identity, unit).status.value == "COMPLETE"
