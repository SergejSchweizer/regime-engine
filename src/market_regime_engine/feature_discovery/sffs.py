...
def select_sffs(
    ...
    *,
    ...
    for name in candidate_tuple:
        evaluated = _score(score, (name,))
        if evaluated is not None and _better(((name,), evaluated), best, candidate_order):
            best = ((name,), evaluated)
    if best is None:
    ...
    while len(selected) < max_features:
        ...
        for name in candidate_tuple:
            ...
            if proposal in visited:
                continue
            evaluated = _score(score, proposal)
            if evaluated is not None and _better((proposal, evaluated), forward, candidate_order):
        ...
        while len(selected) > 1:
            ...
            for name in selected:
                proposal = tuple(item for item in selected if item != name)
                evaluated = _score(score, proposal)
                if (
