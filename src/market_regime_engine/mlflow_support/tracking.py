...
def _scalar_fold_metrics(fold: WalkForwardFoldResult) -> dict[str, float | None]:
    ...
    normalized_train_loglik = (
        None
        if train_count < 1 or fold.train_log_likelihood is None
        else fold.train_log_likelihood / train_count
    )
    normalized_aic = None if train_count < 1 or fold.aic is None else fold.aic / train_count
    normalized_bic = None if train_count < 1 or fold.bic is None else fold.bic / train_count
    return {
        "fold_train_loglik": normalized_train_loglik,
        ...
        "fold_aic": fold.aic,
        "fold_bic": fold.bic,
        "fold_aic_per_train_obs": normalized_aic,
        "fold_bic_per_train_obs": normalized_bic,
