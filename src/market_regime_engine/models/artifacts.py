...
def _full_covariance(covariance: tuple[tuple[float, ...], ...], dimension: int, name: str) -> None:
    ...
    max_asymmetry = max(
        abs(covariance[i][j] - covariance[j][i]) for i in range(dimension) for j in range(dimension)
    )
...
class GaussianHMMArtifact:
    ...
    def distribution_covariances(self) -> tuple[tuple[tuple[float, ...], ...], ...]:
        ...
        return tuple(
            tuple(tuple(value * degree / (degree - 2.0) for value in row) for row in covariance)
