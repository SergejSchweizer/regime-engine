...
anchor = "Do not compare state-signature RMS distances across different feature spaces.\n\nExecution rule:"
if anchor not in text:
    raise SystemExit('section-18 evaluation anchor missing')

evaluation_contract = r'''Do not compare state-signature RMS distances across different feature spaces.
...
Execution rule:'''
text = text.replace(anchor, evaluation_contract, 1)

insert_marker = "### PR-152 — Track and visualize Xetra v3 shadow analysis in MLflow\n"
if insert_marker not in text:
    raise SystemExit('PR-152 marker missing')

new_prs = r'''### PR-159 — Pin the three evaluation identities and champion rules
...
'''
text = text.replace(insert_marker, new_prs + insert_marker, 1)

start = text.index("### PR-152 — Track and visualize Xetra v3 shadow analysis in MLflow\n")
end = text.index("### PR-153 — Add standalone Xetra v3 shadow-analysis runner\n", start)
pr152 = r'''### PR-152 — Track the three Xetra v3 evaluations in MLflow with local statistics parity
...
'''
text = text[:start] + pr152 + text[end:]

start = text.index("### PR-153 — Add standalone Xetra v3 shadow-analysis runner\n")
end = text.index("### PR-154 — Prove 61-feature selection and shadow workflow hermetically\n", start)
pr153 = r'''### PR-153 — Add standalone Xetra v3 three-evaluation runner
...
if old_graph not in text:
    raise SystemExit('execution graph marker missing')
...
tail_new = tail + " PR-159 formalizes the three evaluation/champion namespaces after agreement mathematics exists; PR-160 is an isolated filesystem-evidence component; PR-152 is the first PR allowed to bind those contracts to MLflow."
if tail not in text:
    raise SystemExit('graph explanatory tail missing')
