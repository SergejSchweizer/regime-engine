...
| PR-426 | IN PROGRESS (IMPLEMENTATION MERGED #418; projection QA #422; PR-439/GitHub #427 open) | Per-K Model Metrics and plot payload contracts now fail closed on incomplete lineage and prove serial/process-order canonical parity; positive dimension-independent Cross-K projection, unavailable-slot manifest behavior and exact alias absence are now covered locally; the full artifact projection matrix remains |
...
GitHub PR #427 (`pr/PR-439-k-plot-projection-qa`) is open; the full artifact
projection matrix and external production acceptance remain open.
