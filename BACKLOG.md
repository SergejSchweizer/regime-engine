...
- **Latest implementation:** the parallel audit dossier handoff is complete
  ...
  process-parallel work. Follow-up PRs #333–#343 are also merged: global
...
- **Current CPU implementation:** the runtime uses affinity/cgroup-aware
  ...
  crash-boundary hardening is included through PRs #259–#343.
...
- **Remote branch/PR state:** GitHub PRs #270–#343 are merged except #277,
  #284 and #317, which are closed without merge; no GitHub PRs are open and no
  `pr/*` remote branches remain. The implementation branches for #303–#343
...
- **Latest verification:** durable-run, source-resume, stage-checkpoint,
  ...
  (`589 passed` for the non-integration/non-external selector). The corrected hermetic full-computation proof
...
update. GitHub has no open PRs. PRs #270–#343
...
| PR-231 | IMPLEMENTATION MERGED | Acceptance open: hermetic proof closure |
| PR-232 | IMPLEMENTATION MERGED | Acceptance open: full current-Xetra computation and independent audit |
...
| PR-250 | IMPLEMENTATION MERGED | Acceptance open: Model Metrics completeness and resume parity |
| PR-252 | IMPLEMENTED | Cleanup is conditional/no-op when inventory is empty |
| PR-253 | IMPLEMENTATION MERGED | Acceptance open: combined zero-legacy/runtime audit |
...
| PR-267 | IMPLEMENTATION MERGED | Acceptance/quality closure remains tracked in the PR-267 contract |
...
PRs **#270–#343** are merged except **#277, #284 and #317**, which are
...
The implementation is now complete on `main` through GitHub PRs #303–#313;
the status ledger above is authoritative. The planning acceptance text below
is retained for traceability and is not an open implementation queue.
No PCA implementation, configuration, source adapter or PCA-specific test was
found in the current codebase. That earlier conclusion is superseded: all
eight PCA planning items are implemented on `main` and mapped in the status
ledger above.
...
- **Status:** open; no PCA transformer or immutable PCA artifact is implemented.
- **Open work:** TRAIN-only standardization/PCA fitting, deterministic component
  signs and retained-`k` selection at cumulative explained variance >= 90%.
...
- **Status:** open; PCA source selection and frozen inner-TRAIN fit scope are
  not implemented.
...
- **Status:** open; `pca_pc_001 ... pca_pc_k` generation, provenance and catalog
...
- **Status:** open; raw and generated PCA features are not combined in the v4
  distance, clustering, scoring or selection pipeline.
...
- **Status:** open; fold-local PCA artifacts and the two-stage PCA/HMM scaling
...
- **Status:** open; production package, latest/replay serving and PCA lineage
  validation do not support PCA features.
...
- **Status:** open; PCA loading/EVR metrics, plots and matched OOS comparison
  are not tracked.
...
- **Status:** open; PCA configuration, E2E proof and documentation closure are
  not implemented.
