...
The trusted-LAN PostgreSQL server at the canonical endpoint does not offer TLS. Production therefore requires explicit plaintext transport through `sslmode=disable`; PR-033 external verification must prove the connection is non-TLS, read-only and least-privilege. An implementation must not silently change this transport setting.
